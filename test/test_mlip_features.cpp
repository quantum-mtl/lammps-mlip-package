...
TEST(MLIPFeaturesTest, get_lm_info_table) {
    ...
    // list of (l, m, offset + id of m, offset + id of -m) where m < 0, offset =
    // l * l
    vector2i expect;
...
TEST(MLIPFeaturesTest, get_ylm) {
    ...
    auto expect = vector1dc{
        sqrt(1.0 / (4.0 * M_PI)), 0, sqrt(3 / (4 * M_PI)), 0, 0,
        sqrt(5 / (4 * M_PI)),
    };
...
// Broken test
// https://github.com/google/googletest/blob/master/docs/advanced.md#temporarily-disabling-tests
