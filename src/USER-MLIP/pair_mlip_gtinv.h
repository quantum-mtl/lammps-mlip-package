...
class PairMLIPGtinv : public Pair {
   ...
   protected:
    ...
    vector2dc compute_anlm_uniq_products(const int &type1,
    ...
    void compute_partial_anlm_product_for_each_atom(
        const int n_fn, const int n_lm_all, const barray4dc &anlm, int ii,
        barray4dc &prod_anlm_f, barray4dc &prod_anlm_e);
    void update_partial_anlm_product_of_Iatom(
        const int n_fn, const int n_lm_all, barray4dc &prod_anlm_f,
        barray4dc &prod_anlm_e, int i, int type1, const tagint *tag,
        const int n_gtinv, const vector2dc &uniq, const vector1d &uniq_p);
    void compute_anlm_linear_term(const int n_gtinv, const int tc0, int n,
                                  int lm0, const vector2dc &uniq, dc &sumf,
                                  dc &sume);
    ...
    void compute_energy_and_force_for_each_atom(
