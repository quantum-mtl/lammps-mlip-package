...
template<typename T>
T DataMLIP::get_value(std::ifstream& input) {
    std::string line;
    std::stringstream ss;
...
template<typename T>
std::vector<T> DataMLIP::get_value_array
...
void DataMLIP::initialize(char* file) {
    ...
    poly_gtinv_ = PolynomialGtinv(fp_, modelp_, lm_info_);
...
const feature_params& DataMLIP::get_feature_params() const {
    return fp_;
}

const ModelParams& DataMLIP::get_model_params() const {
    return modelp_;
}

const vector2i& DataMLIP::get_lm_info() const {
    return lm_info_;
}

const PolynomialGtinv& DataMLIP::get_poly_gtinv() const {
    return poly_gtinv_;
}

const vector1d& DataMLIP::get_reg_coeffs() const {
    return reg_coeffs_;
}

const std::vector<std::string>& DataMLIP::get_elements() const {
    return ele_;
}

const vector1d& DataMLIP::get_masses() const {
    return mass_;
}

const vector2i& DataMLIP::get_type_comb() const {
    return type_comb_;
}
