...
PairMLIPPairKokkos<DeviceType>::PairMLIPPairKokkos(LAMMPS *lmp)
    : PairMLIPPair(lmp) {
    respa_enable = 0;
    single_enable = 0;
...
void PairMLIPPairKokkos<DeviceType>::init_style() {
    ...
    if (neighflag == HALF || neighflag == HALFTHREAD) {
        ...
        error->all(FLERR,
                   "Must use half neighbor list style with pair mlip_pair/kk");
    }
}
...
void PairMLIPPairKokkos<DeviceType>::compute(int eflag_in, int vflag_in) {
...

    {
        ...
        const int n_type_comb =
            pot.get_model_params().get_type_comb_pair().size();
        ...
        if (pot.get_feature_params().maxp > 1) {
            vector2d dn(inum,
                        vector1d(pot.get_model_params().get_n_des(), 0.0));
#ifdef _OPENMP
...
#endif
            for (int ii = 0; ii < inum; ii++) {
                compute_partial_structural_feature_for_each_atom(
                    dn, ii, prod_all_f, prod_all_e);
            }
        }
...
#endif
        for (int ii = 0; ii < inum; ii++) {
            compute_energy_and_force_for_each_atom(prod_all_f, prod_all_e, ii,
                                                   evdwl_array, fpair_array);
        }
        accumulate_energy_and_force_for_all_atom(inum, nlocal, newton_pair,
                                                 evdwl_array, fpair_array);
    }
...
template <class DeviceType>
void PairMLIPPairKokkos<
    DeviceType>::compute_main_structural_feature_for_each_atom(vector2d &dn,
                                                               int ii) {
...
template <class DeviceType>
void PairMLIPPairKokkos<DeviceType>::
    compute_partial_structural_feature_for_each_atom(const vector2d &dn, int ii,
                                                     vector3d &prod_all_f,
                                                     vector3d &prod_all_e) {
    int i, *ilist, type1;
    double **x = atom->x;
    ...
    const int n_fn = pot.get_model_params().get_n_fn();
    const vector1d &prodi = polynomial_model_uniq_products(dn[tag[i] - 1]);
...
template <class DeviceType>
void PairMLIPPairKokkos<DeviceType>::compute_energy_and_force_for_each_atom(
    const vector3d &prod_all_f, const vector3d &prod_all_e, int ii,
...
template <class DeviceType>
void PairMLIPPairKokkos<DeviceType>::accumulate_energy_and_force_for_all_atom(
    ...
    for (int ii = 0; ii < inum; ii++) {
        ...
        for (int jj = 0; jj < jnum; jj++) {
            ...
            if (dis < pot.get_feature_params().cutoff) {
                ...
                if (evflag) {
                    ev_tally(i, j, nlocal, newton_pair, evdwl, ecoul, fpair,
                             delx, dely, delz);
                }
            }
        }
