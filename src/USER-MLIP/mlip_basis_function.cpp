...
void bump_cutoff_function_d(const double& dis, const double& cutoff, double& bf,
                            double& bf_d) {
    double x = dis / cutoff;
...
void gauss_d(const double& dis, const double& p1, const double& p2, double& bf,
             double& bf_d) {
    bf = gauss(dis, p1, p2);
    bf_d = -2.0 * p1 * (dis - p2) * bf;
}
double cosine(const double& dis, const double& p1) { return cos(p1 * dis); }
void cosine_d(const double& dis, const double& p1, double& bf, double& bf_d) {
    bf = cosine(dis, p1);
    bf_d = -p1 * sin(p1 * dis);
}
double sine(const double& dis, const double& p1) { return sin(p1 * dis); }
...
void sph_bessel_d(const double& dis, const double& p1, const double& p2,
    ...
    if (index == 0)
        bf_d = -boost::math::sph_bessel(1, pdis) * p2;
    else {
...
void sph_neumann_d(const double& dis, const double& p1, double& bf,
                   double& bf_d) {
    int index = round(p1);
    bf = boost::math::sph_neumann(index, dis);
    if (index == 0)
        bf_d = -boost::math::sph_neumann(1, dis);
    else
        bf_d =
            boost::math::sph_neumann(index - 1, dis) - (index + 1) * bf / dis;
}

double polynomial(const double& dis, const double& param1) {
    return pow(dis, param1);
}
void polynomial_d(const double& dis, const double& param1, double& bf,
                  double& bf_d) {
...
void sto_d(const double& dis, const double& param1, const double& param2,
           double& bf, double& bf_d) {
...
void gto_d(const double& dis, const double& param1, const double& param2,
           double& bf, double& bf_d) {
...
void morlet_d(const double& dis, const double& param1, double& bf,
              double& bf_d) {
...
void modified_morlet_d(const double& dis, const double& param1, double& bf,
                       double& bf_d) {
...
void mexican_d(const double& dis, const double& param1, double& bf,
               double& bf_d) {
    double prod1 = pow(dis, 2.0) * pow(param1, 2.0);
    double prod2 = exp(-0.5 * prod1);
...
void morse_d(const double& dis, const double& param1, const double& param2,
             double& bf, double& bf_d) {
    double prod1 = -param1 * (dis - param2);
