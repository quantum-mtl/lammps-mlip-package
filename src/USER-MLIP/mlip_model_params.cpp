...
ModelParams::ModelParams(const struct feature_params& fp) {
    ...
    if (n_type == 1)
        ...
        type_comb_pair = {{{0}, {}}, {{1}, {0}}, {{}, {1}}};

    if (fp.des_type == "pair")
        n_des = n_fn * type_comb_pair.size();
    ...
    if (fp.model_type == 2) {
        for (int n = 0; n < n_des; ++n) polynomial_index.emplace_back(n);
        ...
        for (int i = 0; i < linear_array_g.size(); ++i) {
            ...
            if (lin.tcomb_index.size() == 1) {
                for (int n = 0; n < n_fn; ++n) {
                    polynomial_index.emplace_back(n * linear_array_g.size() +
                                                  i);
                }
            }
        }
...
int ModelParams::seq2typecomb(const int& seq) { return seq / n_fn; }
    ...
    for (int i1 = 0; i1 < iarray.size(); ++i1) {
        ...
        for (int i2 = 0; i2 <= i1; ++i2) {
            ...
            for (int i3 = 0; i3 <= i2; ++i3) {
                ...
                for (int type1 = 0; type1 < n_type; ++type1) {
                    if (check_type_comb_pair(vector1i({t1, t2, t3}), type1) ==
                        true) {
                        comb3.push_back(
                            vector1i({iarray[i3], iarray[i2], iarray[i1]}));
                        break;
...
bool ModelParams::check_type_comb_pair(const vector1i& index,
                                       const int& type1) const {
    vector1i size;
...
const std::vector<struct LinearTermGtinv>& ModelParams::get_linear_term_gtinv()
    const {
