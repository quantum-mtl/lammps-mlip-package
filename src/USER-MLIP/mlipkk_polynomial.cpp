                                 ...
                                 n_itc_(static_cast<int>(irreps_type_pairs.size()))
{
    ...
    // set polynomial_index_, polynomial_index_mapping, polynomial_typecomb_intersection_, and n_coeffs_
    if (model_type == 1) {
...
void MLIPPolynomial::enumerate_polynomials_model_type_1(const std::vector<IrrepsTypePair>& irreps_type_pairs) {
...
void MLIPPolynomial::enumerate_polynomials_model_type_2(const std::vector<IrrepsTypePair>& irreps_type_pairs) {
    int n_des = n_itc_ * n_fn_;
    ...
    for (int poly_order = 1; poly_order <= maxp_; ++poly_order) {
        ...
        for (std::vector<FeatureIdx> fcomb: all_combs) {
            // check if intersection of type combs is not empty
            const auto intersection = get_typecombs_intersection(fcomb, irreps_type_pairs);
            if (std::any_of(intersection.begin(), intersection.end(), [](const bool e){ return e; })) {
                polynomial_index_.emplace_back(fcomb);
                for (ElementType type = 0; type < n_types_; ++type) {
                    polynomial_typecomb_intersection_[type].emplace_back(intersection[type]);
...
void MLIPPolynomial::enumerate_polynomials_model_type_3(const std::vector<IrrepsTypePair>& irreps_type_pairs) {
    int n_des = n_itc_ * n_fn_;
    ...
    for (FeatureIdx fidx = 0; fidx < n_des; ++fidx) {
        const IrrepsTypeCombIdx itcidx = get_irreps_type_idx(fidx);
        const int gtinv_order = static_cast<int>(irreps_type_pairs[itcidx].type_combs.size());
        if (gtinv_order == 1) {
            pair_feature_indices.emplace_back(fidx);
        }
    }

    enumerate_polynomials_model_type_3_or_4(irreps_type_pairs, pair_feature_indices);
}

void MLIPPolynomial::enumerate_polynomials_model_type_4(const std::vector<IrrepsTypePair>& irreps_type_pairs) {
    int n_des = n_itc_ * n_fn_;
    ...
    enumerate_polynomials_model_type_3_or_4(irreps_type_pairs, lower_feature_indices);
}
    ...
    const std::vector<FeatureIdx>& lower_feature_indices)
{
    ...
    for (int poly_order = 2; poly_order <= maxp_; ++poly_order) {
        // do not use get_combinations_with_repetition for backward compatibility!
        auto all_mapping_combs = get_combinations_with_repetition_gtinv(static_cast<int>(lower_feature_indices.size()), poly_order);
