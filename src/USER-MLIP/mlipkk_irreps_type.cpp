...
std::vector<IrrepsTypePair> get_unique_irreps_type_pairs(
    ...
    int max_order =
        static_cast<int>(l_array[static_cast<int>(l_array.size()) - 1].size());
    std::vector<std::vector<std::vector<TypeCombIdx>>> uniq_type_combs(
        max_order);
    for (int order = 1; order <= max_order; ++order) {
        std::vector<std::vector<TypeCombIdx>> all_combs =
            get_combinations_with_repetition(num_type_combs, order);
        for (auto comb : all_combs) {
            // check if intersection of types is not empty
            auto intersection =
                get_type_intersection(n_types, type_pairs_mapping, comb);
    ...
    for (IrrepsIdx i = 0; i < static_cast<int>(l_array.size()); ++i) {
        ...
        for (const auto& irreps_types : set_irreps_types) {
            ...
            auto intersection =
                get_type_intersection(n_types, type_pairs_mapping, type_comb);
            irreps_types_pairs.emplace_back(
                IrrepsTypePair{i, type_comb, intersection});
        }
    }
