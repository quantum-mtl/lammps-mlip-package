...
class Readgtinv {
   ...
   public:
    ...
    /*
    Table of sets of angular numbers corresponding to all symmetry Irreps to be
    used. Note that some angular sets (e.g. {1, 1, 1, 1}) are duplicated because
    ...
    */
    ...
    l_array_all[i] coeffs_all: IrrepsIdx(int) -> vector<double>
    */
    ...
    /* irreps_term_mapping_: IrrepsTermIdx -> IrrepsIdx */
    const std::vector<IrrepsIdx> get_irreps_term_mapping() const {
        return irreps_term_mapping_;
    };
    const vector1i& get_irreps_first_term() const {
        return irreps_first_term_;
    };
