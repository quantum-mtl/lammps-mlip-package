                 ...
                 view_2d d_alp)
{
    ...
    for (int l = 2; l <= maxl; ++l) {
        for (int m = 0; m <= l - 2; ++m) {
            // DLMF 14.10.3
            d_alp(npidx, lm2i(l, m)) = d_sph_coeffs_A(lm2i(l, m))
                                    * (costheta * d_alp(npidx, lm2i(l - 1, m)) + d_sph_coeffs_B(lm2i(l, m)) * d_alp(npidx, lm2i(l - 2, m)));
        }
        d_alp(npidx, lm2i(l, l - 1)) = costheta * sqrt(2.0 * (l - 1.0) + 3.0) * tmp;
...
KOKKOS_INLINE_FUNCTION
void compute_ylm(const NeighborPairIdx npidx, const double azimuthal, const int maxl,
                 const view_2d d_alp, view_2dc d_ylm)
{
    for (int l = 0; l <= maxl; ++l) {
        d_ylm(npidx, lm2i(l, l)) = d_alp(npidx, lm2i(l, 0)) * 0.5 * M_SQRT2;  // (l, m=0)
    }
    ...
    for (int mp = 1; mp <= maxl; ++mp) {
        ...
        for (int l = mp; l <= maxl; ++l) {
            ...
            d_ylm(npidx, lm2i(l, l - mp)) = tmp * Kokkos::complex<double>(c, -s);  // (l, m=-mp)
        }
        sign *= -1;
    }
}
                          ...
                          view_2d d_alp_sintheta)
{
    ...
    for (int l = 2; l <= maxl; ++l) {
        for (int m = 1; m <= l - 2; ++m) {
            // DLMF 14.10.3
            d_alp_sintheta(npidx, lm2i(l, m)) = d_sph_coeffs_A(lm2i(l, m))
                            * (costheta * d_alp_sintheta(npidx, lm2i(l - 1, m))
                               + d_sph_coeffs_B(lm2i(l, m)) * d_alp_sintheta(npidx, lm2i(l - 2, m)));
        }
        // DLMF
        d_alp_sintheta(npidx, lm2i(l, l - 1)) = costheta * sqrt(2.0 * (l - 1.0) + 3.0) * tmp;
        tmp *= -sqrt(1.0 + 0.5 / l) * sintheta;
        // DLMF 14.7.15
...
KOKKOS_INLINE_FUNCTION
void compute_ylm_der(const NeighborPairIdx npidx, const double costheta, const double azimuthal, const double r,
                     const int maxl, const view_2d d_alp_sintheta,
                     view_2dc d_ylm_dx, view_2dc d_ylm_dy, view_2dc d_ylm_dz)
{
    const double sintheta = sqrt(1.0 - costheta * costheta);
    ...
    // (l, 0)
    for (int l = 0; l <= maxl; ++l) {
        const double common = d_alp_sintheta(npidx, lm2i(l, 1)) * sintheta * invr * sqrt(0.5 * l * (l + 1));
    ...
    for (int mp = 1; mp <= maxl; ++mp) {
        ...
        for (int l = mp; l <= maxl; ++l) {
            ...
            if (mp != l) {
                dtheta += sqrt(static_cast<double>(l - mp) * (l + mp + 1)) * d_alp_sintheta(npidx, lm2i(l, mp + 1)) * sintheta;  // TODO: reuse p[]
            }
            const Kokkos::complex<double> dphi(0.0, mp * d_alp_sintheta(npidx, lm2i(l, mp)));

            d_ylm_dx(npidx, lm2i(l, l - mp)) = sign * conj(common * (dtheta * costheta * cosphi - dphi * sinphi));
            d_ylm_dy(npidx, lm2i(l, l - mp)) = sign * conj(common * (dtheta * costheta * sinphi + dphi * cosphi));
            d_ylm_dz(npidx, lm2i(l, l - mp)) = sign * conj(-common * dtheta * sintheta);
        }
        sign *= -1.0;
    }
}
