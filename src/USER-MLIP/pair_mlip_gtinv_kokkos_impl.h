...
PairMLIPGtinvKokkos<DeviceType>::PairMLIPGtinvKokkos(LAMMPS *lmp)
    : PairMLIPGtinv(lmp) {
    ...
    Pair::datamask_read =
        X_MASK | F_MASK | TYPE_MASK | ENERGY_MASK | VIRIAL_MASK;
...
void PairMLIPGtinvKokkos<DeviceType>::coeff(int narg, char **arg) {
...

    {
        ...
        MLIP_NS::read_potential_file(arg[2], ele, mass, fp, reg_coeffs,
                                     gtinvdata);
#ifdef _DEBUG
        ...
        model = new MLIP_NS::MLIPModelLMP<PairMLIPGtinvKokkos<DeviceType>,
...

    {
        ...
        for (i = 3; i < narg; i++) {
            ...
            if (j < ele.size())
                ...
                error->all(FLERR,
                           "No matching element in Machine Learning "
...

    {
        ...
        for (i = 1; i <= n; i++)
            for (j = i; j <= n; j++) Pair::setflag[i][j] = 0;
    }
    ...
    if (lmp->kokkos->neighflag == HALF ||
        lmp->kokkos->neighflag == HALFTHREAD) {
        ...
        for (i = 0; i < n; ++i) {
            types.template emplace_back(
                map[(atom->type)[i]]);  // atom->type[] and map[] starts from 0
                                        // and 1, respectively
        }
    }
}
...
double PairMLIPGtinvKokkos<DeviceType>::init_one(int i, int j) {
    //! DO NOT CALL PairMLIPGtinv::init_one(). It requires pot.get_cutmax(), but
    //! we have no `pot`.
...
void PairMLIPGtinvKokkos<DeviceType>::init_style() {
    ...
    // This newton-on check will be deleted when derivatives of site-energy are
    // communicated in `compute()`.
    ...
    if (neighflag == FULL) {     // neigh full
        if (newton_pair == 0) {  // newton off
            error->all(FLERR,
                       "Must use 'neigh full newton on' with mlip_gtinv/kk");
        } else {  // newton on
        ...
        error->all(FLERR,
                   "Cannot (yet) use neigh half with mlip_gtinv/kk; run with "
                   "'-pk kokkos neigh full newton on'");
    }
}
...
void PairMLIPGtinvKokkos<DeviceType>::compute(int eflag_in, int vflag_in) {
    ...
    //  if (vflag_fdotr && !no_virial_fdotr_compute)
    //  pair_virial_fdotr_compute(this);  // always false
    copymode = 0;
...
void MLIPModelLMP<PairStyle, NeighListKokkos>::set_structure(
    PairStyle *fpair, NeighListKokkos *k_list) {
    ...
    nall_ = fpair->atomKK->nlocal +
            fpair->atomKK
                ->nghost;  // Total number of owned and ghost atoms on this proc
    ...
    if (neighflag_ == FULL) {
            ...
            if (i < nlocal_) {
                ...
                for (int jj = 0; jj < num_neighbors_i; ++jj) {
                    ...
                    h_neighbor_pair_index(count_neighbor) =
                        Kokkos::pair<LMPLocalIdx, LMPLocalIdx>(i, j);
                    ++count_neighbor;
                }
            }
        }
        ...
        for (int ii = 0; ii < inum_; ++ii) {
            ...
            for (int jj = 0; jj < num_neighbors_i; ++jj) {
                ...
                h_neighbor_pair_index(count_neighbor) =
                    Kokkos::pair<LMPAtomID, LMPAtomID>(site_i, site_j);
                ++count_neighbor;
            }
        }
    }
    ...
    for (int ii = 0; ii < inum_; ++ii) {
        ...
        if (i < nlocal_) {
            ...
            for (int jj = 0; jj < num_neighbors_i; ++jj) {
                LMPLocalIdx j = h_neighbors(i, jj);
                j &= NEIGHMASK;
                h_neighbor_pair_displacements(count_neighbor, 0) =
                    h_x(j, 0) - xtmp;
                h_neighbor_pair_displacements(count_neighbor, 1) =
                    h_x(j, 1) - ytmp;
                h_neighbor_pair_displacements(count_neighbor, 2) =
                    h_x(j, 2) - ztmp;
                ++count_neighbor;
            }
        }
    ...
    if (neighflag_ == FULL) {
        ...
        for (NeighborPairIdx npidx = 0; npidx < n_pairs_; ++npidx) {
            ...
            h_neighbor_pair_typecomb(npidx) =
                type_pairs_kk_.h_view(type_i, type_j);
        }
    } else if (neighflag_ == HALF || neighflag_ == HALFTHREAD) {
        for (NeighborPairIdx npidx = 0; npidx < n_pairs_; ++npidx) {
            const auto &ij = h_neighbor_pair_index(npidx);
    ...
    if (neighflag_ == FULL) {
        ...
        for (SiteIdx i = 0; i < nall_; i++) {
            // Element types (int) loaded from in-file is mapped into MLIP
            // potential index:
    ...
    Kokkos::realloc(d_basis_function_adjoints_, nlocal_, n_typecomb_, n_fn_,
                    n_lm_half_);

    Kokkos::realloc(site_energy_kk_, nlocal_);
    if (fpair->neighflag == FULL) {
        // FULL stores forces on ghost atoms to be communicated thus View should
        // be allocated for all atoms on the proc.
...
template <class PairStyle, class NeighListKokkos>
void MLIPModelLMP<PairStyle, NeighListKokkos>::get_forces(
    PairStyle *fpair, NeighListKokkos *k_list) {
    ...
    if (fpair->neighflag == FULL) {
        ...
        // Copy i-th force of the model to the center atom of i-th neighbor
        // list.
        fpair->atomKK->k_tag.sync_host();
        auto h_tag = fpair->atomKK->k_tag.view_host();
        for (int ii = 0; ii < inum_; ++ii) {
