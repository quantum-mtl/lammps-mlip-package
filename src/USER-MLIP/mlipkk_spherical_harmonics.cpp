...
SphericalHarmonics& SphericalHarmonics::operator=(
    const SphericalHarmonics& other) {
    if (this != &other) {
...
/// @brief spherical harmonics for (l, m) = (0, 0), (1, -1), (1, 0), (2, -2),
/// ..., (lmax, 0) Be careful returned list does not contain m > 0
/// @param[in] costheta cosine of polar angle
/// @param[in] azimuthal [0, 2 * pi)
/// @param[out] ylm
void SphericalHarmonics::compute_ylm(const double costheta,
    ...
    for (int mp = 1; mp <= lmax_; ++mp) {
        ...
        for (int l = mp; l <= lmax_; ++l) {
            ...
            ylm[lm2i(l, l - mp)] =
                tmp * std::complex<double>(c, -s);  // (l, m=-mp)
        }
        sign *= -1;
    }
}
...
/// @param[out] ylm_dy
/// @param[out] ylm_dz
void SphericalHarmonics::compute_ylm_der(const double costheta,
                                         const double azimuthal, const double r,
                                         vector1dc& ylm_dx, vector1dc& ylm_dy,
    ...
    for (int l = 0; l <= lmax_; ++l) {
        const double common =
            q[lm2i(l, 1)] * sintheta * invr * sqrt(0.5 * l * (l + 1));
    ...
    for (int mp = 1; mp <= lmax_; ++mp) {
        ...
        for (int l = mp; l <= lmax_; ++l) {
            ...
            if (mp != l) {
                dtheta += sqrt((l - mp) * (l + mp + 1)) * q[lm2i(l, mp + 1)] *
                          sintheta;  // TODO: reuse p[]
            }
            const dc dphi(0.0, mp * q[lm2i(l, mp)]);

            ylm_dx[lm2i(l, l - mp)] =
                sign * std::conj(common *
                                 (dtheta * costheta * cosphi - dphi * sinphi));
            ylm_dy[lm2i(l, l - mp)] =
                sign * std::conj(common *
                                 (dtheta * costheta * sinphi + dphi * cosphi));
            ylm_dz[lm2i(l, l - mp)] =
                sign * std::conj(-common * dtheta * sintheta);
        }
        sign *= -1.0;
    }
}
...
void SphericalHarmonics::compute_normalized_associated_legendre(
    ...
    for (int l = 2; l <= lmax_; ++l) {
        for (int m = 0; m <= l - 2; ++m) {
            // DLMF 14.10.3
            p[lm2i(l, m)] =
                A_[lm2i(l, m)] * (costheta * p[lm2i(l - 1, m)] +
...
/// P_{l}^{m}/sintheta
void SphericalHarmonics::compute_normalized_associated_legendre_sintheta(
    const double costheta, vector1d& q) const {
    ...
    for (int l = 2; l <= lmax_; ++l) {
        for (int m = 1; m <= l - 2; ++m) {
            // DLMF 14.10.3
            q[lm2i(l, m)] =
                A_[lm2i(l, m)] * (costheta * q[lm2i(l - 1, m)] +
