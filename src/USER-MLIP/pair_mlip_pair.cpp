...
PairMLIPPair::PairMLIPPair(LAMMPS *lmp) : Pair(lmp) { restartinfo = 0; }
...
#endif
        for (int ii = 0; ii < inum; ii++) {
            compute_partial_structural_feature_for_each_atom(dn, ii, prod_all_f,
                                                             prod_all_e);
        }
    }
...
void PairMLIPPair::compute_main_structural_feature_for_each_atom(vector2d &dn,
                                                                 int ii) {
...
void PairMLIPPair::compute_partial_structural_feature_for_each_atom(
    const vector2d &dn, int ii, vector3d &prod_all_f, vector3d &prod_all_e) {
    int i, *ilist, type1;
    double **x = atom->x;
    tagint *tag = atom->tag;
    ilist = list->ilist;
...
void PairMLIPPair::compute_energy_and_force_for_each_atom(
...
void PairMLIPPair::accumulate_energy_and_force_for_all_atom(
    ...
    for (int ii = 0; ii < inum; ii++) {
        ...
        for (int jj = 0; jj < jnum; jj++) {
            ...
            if (dis < pot.get_feature_params().cutoff) {
                ...
                f[j][2] -= fpair * delz;
                if (evflag) {
                    ev_tally(i, j, nlocal, newton_pair, evdwl, 0.0, fpair, delx,
                             dely, delz);
                }
            }
        }
...
double PairMLIPPair::dot(const vector1d &a, const vector1d &b,
                         const int &sindex) {
    double val(0.0);
